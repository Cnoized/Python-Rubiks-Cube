...
#    "Turns the bot row right."
    ...
    def CallBack(self):
        ...
        colorCube = [[["white" if n==1 else "green" if n==2 else "yellow" if n==3 else "blue" if n==4 else "red" if n==5 else "orange" for n in a] for a in subset] for subset in self.cube]
        for n in range(6):
            for m in range(3):
                for l in range(3):
...
#                    else:
#                        self.cCube[n][m][l] = 0
                    "Possible implementation of self.cCube."
                    label = Label(root, text = self.cube[n][m][l],bg = colorCube[n][m][l],fg = colorCube[n][m][l])
...
#        self.cCube = self.cube == [[[n]*3]*3 for n in range(1,7)]
        ...
        "Multiply the 'boolian' values of each cubes' faces to find out if you add one to the number of current correct cubes."
...
#            print(self.cCube)
#            print(self.correct)
            "Debug code for indexing in array."
            print("correct faces:",sum(sum(sum(self.cCube,[]),[])))
            print("correct cubes:",sum(self.correct))
        self.AiArray()
...
#        label.grid(sticky=N+S+E+W, row=6,column=3,columnspan=2)
    ...
    def AiArray(self):
        self.aiArray = [[0]*6]*54
        self.aiArray = [[1 if self.cube[n][m][l] == a+1 else 0 for a in range(6)] for n in range(6) for l in range(3) for m in range(3)]
        if self.supress==False:
            print(self.aiArray)
    __call__ = AiArray
    "Created self.AiArray to make qualitative analysis of cube posistions at a given time possible." 
    def Random(self):
    ...
    def Turn20(self):
        for n in range(21):
            self.Random()
    "Randomize cube. Currently allows for repeats and adjacent/consecutive inverses."
